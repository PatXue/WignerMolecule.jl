...
function Carlo.measure!(mc::WignerMC, ctx::Carlo.MCContext)
    ...
    measure!(ctx, :ηk_K, K(mc.ηks))
    measure!(ctx, :ηk_half_K, half_K(mc.ηks))
    ...
    measure!(ctx, :ηk_corr_K, K(mc.ηks) * K(mc.ηks)')
    measure!(ctx, :ηk_corr_half_K, half_K(mc.ηks) * half_K(mc.ηks)')
    measure!(ctx, :sk_Γ, Γ(mc.spinks))
    measure!(ctx, :sk_M, M(mc.spinks))
    measure!(ctx, :sk_half_M, half_M(mc.spinks))
    measure!(ctx, :sk_K, K(mc.spinks))
    measure!(ctx, :sk_half_K, half_K(mc.spinks))
    ...
    measure!(ctx, :sk_corr_K, norm2(K(mc.spinks)))
    measure!(ctx, :sk_corr_half_K, norm2(half_K(mc.spinks)))
