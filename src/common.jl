...
function Carlo.measure!(mc::WignerMC, ctx::Carlo.MCContext)
    ...
    measure!(ctx, :sk_corr_Γ, abs2(Γ(mc.spinks)))
    measure!(ctx, :sk_corr_M, abs2(M(mc.spinks)))
    measure!(ctx, :sk_corr_half_M, abs2(half_M(mc.spinks)))
    measure!(ctx, :sk_corr_path, abs2.(spink_path))
